...
int main(int argc, char *argv[]) {
	...
	pthread_mutex_t one_m, two_m;
	if(pthread_mutex_init(&one_m, NULL) != 0) {
		printf("mutex init1 failed\n");
		return 1;
	}
	if(pthread_mutex_init(&two_m, NULL) != 0) {
		printf("mutex init2 failed\n");
		return 1;
	}


	//one_args.mutex = one_m;
	//two_args.mutex = two_m;
